...
describe TelegramDispatcher, type: :service do
  ...
  before do
    allow(TelegramMessenger).to receive(:send_message).and_return(message)
  end
  ...
  context 'when game has not been created' do
    ...
    context 'when /level command received with selected level' do
      let!(:message) { build :message, :with_realm, realm: realm }
      let!(:callback) { build :callback, data: '/level easy', message: message }
    ...
    context 'when /lang command received with selected language' do
      let!(:message) { build :message, :with_realm, realm: realm }
      let!(:callback) { build :callback, data: '/lang RU', message: message }
  ...
  context 'when game has finished' do
    let!(:game) { create(:game, :realm, :with_tries, secret: 'secret', status: :finished, realm: realm) }
